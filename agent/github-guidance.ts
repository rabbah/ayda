/**
 * System-prompt guidance so the model uses Ayda's managed GitHub flow instead of
 ...
 * security anti-pattern). Ayda instead manages GitHub via a per-user GitHub App
...
export function githubPromptGuidance(opts: { configured: boolean; connected: boolean }): string {
  const base =
    "GitHub access on this agent (called Ayda) is managed for the user. " +
  ...
  return (
    ...
    "account securely through Ayda's GitHub App, with no tokens to paste. " +
